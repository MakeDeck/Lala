...
device.on("pull", function(buffer_len) {
    server.log("agent got pull request - buffer size is: "+buffer_len);
    ...
    server.log("data chunk size "+inParams.data_chunk_size+" Buffer Length: "+ buffer_len);
    ...
    if (inParams.sample_width == 'w') {
        ...
        server.log("Bytes left: "+bytes_left);
    }
    ...
    server.log("buffer length at push: " + buffer.len());
